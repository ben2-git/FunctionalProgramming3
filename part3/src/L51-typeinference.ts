...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    const tvars: T.TVar[] = R.map((exp: A.ClassExp) => exp.typeName, A.parsedToClassExps(parsed));
    const names: string[] = R.map((t: T.TVar) => t.var, tvars);
    return E.makeExtendTEnv(names, tvars, E.makeEmptyTEnv());              
}
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const expVar = exp.var.texp;
    const newTenv = E.makeExtendTEnv([exp.var.var], [expVar], tenv);
    const expVal = typeofExp(exp.val, newTenv);
    const constraint = bind(expVal ,(result: T.TExp) => checkEqualType(result,expVar,exp));
    return bind(constraint, (res:true) => makeOk(T.makeVoidTExp()));
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> => {
    const firstT: Result<T.TExp> = typeofExp(exp,tenv);
    return exps.length === 0 ? firstT : bind(firstT, (x: T.TExp) =>
        A.isDefineExp(exp) ? 
...
export const typeofLit = (exp: A.LitExp): Result<T.TExp> =>{
    return  V.isSymbolSExp(exp.val) ? makeOk(T.makeSymbolTExp(exp.val)) : 
            ...
            makeFailure("Wrong type for literal expression!");
 }
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    const expVar = typeofExp(exp.var, tenv);
    const expVal = typeofExp(exp.val, tenv);
    const constraint1 = safe2((expVar:T.TExp, expVal:T.TExp) => checkEqualType(expVar,expVal,exp))(expVar, expVal);
    return bind(constraint1, (x:true) => makeOk(T.makeVoidTExp()));
    // if not working using apply Tenv on the var like in applycExp
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const paramsTE : T.TExp[] = R.map((v: A.VarDecl) => v.texp, exp.fields);  
    const paramsN : string[] = R.map((v: A.VarDecl) => v.var, exp.fields);
    const methodsTE : Result<T.TExp[]> = mapResult((ex: A.CExp) => typeofExp(ex, E.makeExtendTEnv(paramsN, paramsTE, tenv)), R.map((b: A.Binding) => b.val, exp.methods));
    const methodN : string[] = R.map((b: A.Binding) => b.var.var, exp.methods);
    return bind(methodsTE, (methods: T.TExp[]) => makeOk(T.makeProcTExp(paramsTE, T.makeClassTExp(exp.typeName.var, R.zipWith((name: string, type: T.TExp) => [name, type], methodN, methods)))));
};
