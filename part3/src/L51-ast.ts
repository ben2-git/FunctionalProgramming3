...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> => {
    if(isArray(varDecls) && varDecls.every(isConcreteVarDecl) && isGoodBindings(bindings)){
        const goodTexp: Result<TVar> = bind(parseTExp(typeName), (texp: TExp) => isTVar(texp) ? makeOk(texp) : makeFailure("Wrong name for type of class!"));
        const goodFields: Result<VarDecl[]> = mapResult(parseVarDecl, varDecls);
        const goodMethods: Result<Binding[]> = parseBindings(bindings);
        const constraint: Result<boolean> = bind(goodMethods, (binds: Binding[]) => makeOk(allT(isProcExp, map((b: Binding) => b.val, binds))));
        return bind(constraint, (b: boolean) => b ? safe3((tvar: TVar, vardecls: VarDecl[], binds: Binding[]) => makeOk(makeClassExp(tvar, vardecls, binds))) (goodTexp, goodFields, goodMethods) : makeFailure("Error in class methods!"));
    }
    return makeFailure("Class not written correctly!");
}
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => parsedToClassExpsRec(p, []);

const flatten = (bigarr: ClassExp[][]): ClassExp[] => {
    let ret: ClassExp[] = [];
    for (let i = 0; i < bigarr.length; i++)
        ret = concat(ret, bigarr[i]);
    return ret;
}

const parsedToClassExpsRec = (e: Parsed, acc: ClassExp[]): ClassExp[] => 
    isNumExp(e) ? acc :
    isStrExp(e) ? acc :
    isBoolExp(e) ? acc :
    isPrimOp(e) ? acc :
    isVarRef(e) ? acc :
    // AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp
    isAppExp(e) ? concat(concat(acc, parsedToClassExpsRec(e.rator, [])), flatten(map((rand: CExp) => parsedToClassExpsRec(rand, []), e.rands))) : 
    isIfExp(e) ? concat(acc, parsedToClassExpsRec(e.alt, parsedToClassExpsRec(e.test, parsedToClassExpsRec(e.then, [])))) : 
    isLetExp(e) ? concat(concat(acc, flatten(map((body: CExp) => parsedToClassExpsRec(body, []), e.body))), flatten(map((bind: Binding) => parsedToClassExpsRec(bind.val, []), e.bindings))) : 
    isLetrecExp(e) ? concat(concat(acc, flatten(map((body: CExp) => parsedToClassExpsRec(body, []), e.body))), flatten(map((bind: Binding) => parsedToClassExpsRec(bind.val, []), e.bindings))) : 
    isProcExp(e) ? concat(acc, flatten(map((body: CExp) => parsedToClassExpsRec(body, []), e.body))) : 
    isLitExp(e) ? acc : 
    isSetExp(e) ? concat(acc, parsedToClassExpsRec(e.val, [])) : 
    isClassExp(e) ? concat(acc, [e]) : 
    // DefineExp | Program
    isDefineExp(e) ? concat(acc, parsedToClassExpsRec(e.val, [])) : 
    isProgram(e) ? flatten(map((ex: Exp) => parsedToClassExpsRec(ex, []), e.exps)) : [];
